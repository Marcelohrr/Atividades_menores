import { anoAtual, verificarInputVazio, verificarNumeroInvalido, gerarNumeroAleatorio } from './utils.js';
// Exercício 51
try {
    ...
    form51.addEventListener('submit', function (e) {
        e.preventDefault();
        if (verificarInputVazio([inputPrimeiroTermo51, inputRazao51], resultadoDiv51))
            return;
        const primeiroTermo51 = Number(inputPrimeiroTermo51.value);
        const razao51 = Number(inputRazao51.value);
        if (verificarNumeroInvalido([primeiroTermo51, razao51], resultadoDiv51))
            return;
        const pa51 = [];
        for (let i = 0; i < 10; i++) {
...
try {
    ...
    form52.addEventListener('submit', e => {
        e.preventDefault();
        if (verificarInputVazio([inputNumero52], resultadoDiv52))
            return;
        const numero52 = Number(inputNumero52.value);
        if (verificarNumeroInvalido([numero52], resultadoDiv52))
            return;
        let primo52 = true;
        if (numero52 < 2) {
...
try {
    ...
    form53.addEventListener('submit', e => {
        e.preventDefault();
        if (verificarInputVazio([inputFrase53], resultadoDiv53))
            return;
        const frase53 = inputFrase53.value;
        const fraseFormatada53 = frase53
            .toLowerCase()
            .replace(/\s/g, '');
        const fraseInvertida53 = [];
...
try {
    ...
    const pesos55 = [];
    function registrarPeso55(inputPeso, div) {
        if (pesos55.length === 5) {
            div.textContent = 'Todos os 5 pesos foram inseridos, clique para enviar!';
            return false;
        }
        if (verificarInputVazio([inputPeso], div))
            return false;
        const peso55 = Number(inputPeso.value);
        if (verificarNumeroInvalido([peso55], div))
            return false;
        pesos55.push(peso55);
...
try {
    ...
    form56.addEventListener('submit', e => {
        ...
        let nomeHomemVelho56 = null;
        const mulheresJovens56 = [];
...
try {
    ...
    form58.addEventListener('submit', e => {
        e.preventDefault();
        if (verificarInputVazio([inputNumero58], resultadoDiv58))
            return;
        const numero58 = Number(inputNumero58.value);
        if (verificarNumeroInvalido([numero58], resultadoDiv58))
            return;
...
try {
    ...
    function registrarNumeros59(inputNumero1, inputNumero2, div) {
        if (verificarInputVazio([inputNumero1, inputNumero2], div))
            return null;
        const numero59_1 = Number(inputNumero1.value);
        const numero59_2 = Number(inputNumero2.value);
        if (verificarNumeroInvalido([numero59_1, numero59_2], div))
            return null;
...
try {
    ...
    form60.addEventListener('submit', e => {
        e.preventDefault();
        if (verificarInputVazio([inputNumero60], resultadoDiv60))
            return;
        const numero60 = Number(inputNumero60.value);
        if (verificarNumeroInvalido([numero60], resultadoDiv60))
            return;
        let fatorial60 = 1;
