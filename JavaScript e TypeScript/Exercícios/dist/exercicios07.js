import { real, verificarInputVazio, verificarNumeroInvalido, gerarNumeroAleatorio } from './utils.js';
// Exercício 61
try {
    ...
    form61.addEventListener('submit', e => {
        e.preventDefault();
        if (verificarInputVazio([inputPrimeiroTermo61, inputRazao61], resultadoDiv61))
            return;
        const primeiroTermo61 = Number(inputPrimeiroTermo61.value);
        const razao61 = Number(inputRazao61.value);
        if (verificarNumeroInvalido([primeiroTermo61, razao61], resultadoDiv61))
            return;
...
try {
    ...
    form62.addEventListener('submit', e => {
        ...
        if (verificarInputVazio([inputPrimeiroTermo62, inputRazao62], resultadoDiv62))
            return;
        const primeiroTermo62 = Number(inputPrimeiroTermo62.value);
        const razao62 = Number(inputRazao62.value);
        if (verificarNumeroInvalido([primeiroTermo62, razao62], resultadoDiv62))
            return;
        ...
        button62.addEventListener('click', () => {
            if (verificarInputVazio([input62], div62))
                return;
            const quantidadeTermos62 = Number(input62.value);
            if (verificarNumeroInvalido([quantidadeTermos62], div62))
                return;
...
try {
    ...
    form63.addEventListener('submit', e => {
        e.preventDefault();
        if (verificarInputVazio([inputElementos63], resultadoDiv63))
            return;
        const elementos63 = Number(inputElementos63.value);
        if (verificarNumeroInvalido([elementos63], resultadoDiv63))
            return;
        switch (elementos63) {
            case 1:
...
try {
    ...
    const numeros64 = [];
    function registrarNumero64(input, div) {
        if (verificarInputVazio([input], div))
            return false;
        const numero64 = Number(input.value);
        if (verificarNumeroInvalido([numero64], div))
            return false;
...
try {
    ...
    const numeros65 = [];
    function registrarNumero65(input, div) {
        if (verificarInputVazio([input], div))
            return false;
        const numero65 = Number(input.value);
        if (verificarNumeroInvalido([numero65], div))
            return false;
...
try {
    ...
    form67.addEventListener('submit', e => {
        e.preventDefault();
        if (verificarInputVazio([inputNumero67], resultadoDiv67))
            return;
        const numero67 = Number(inputNumero67.value);
        if (verificarNumeroInvalido([numero67], resultadoDiv67))
            return;
...
try {
    ...
    function verificarParOuImpar68(div) {
...
try {
    ...
    const pessoas69 = [];
    function registrarPessoa69(form, inputIdade, radioGenero, div) {
        if (verificarInputVazio([inputIdade], div))
            return false;
        const idade69 = Number(inputIdade.value);
        if (verificarNumeroInvalido([idade69], div))
            return false;
    ...
    form69.addEventListener('submit', e => {
        ...
        for (const pessoa of pessoas69) {
            ...
            if (pessoa.genero === 'm')
                homens69++;
            if (pessoa.genero === 'f' && pessoa.idade < 20)
                mulheresJovens69++;
        }
        let resposta69_1;
        let resposta69_2;
...
try {
    ...
    const produtos70 = [];
    function registrarProduto70(form, inputProduto, inputPreco, div) {
        if (verificarInputVazio([inputProduto, inputPreco], div))
            return false;
        const produto70 = inputProduto.value;
        const preco70 = Number(inputPreco.value);
        if (verificarNumeroInvalido([preco70], div))
            return false;
        produtos70.push({
            nome: produto70,
            preco: preco70
